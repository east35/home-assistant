...
|---------------|-----------:|-------:|---------|
| `cool_bright` | 100%       | 4291K  | sunrise+30 → sunset-30 (clear weather) |
| `read`        | 80%        | 2890K  | (reserved — currently unused in mood ternary) |
| `relax`       | 56%        | 2237K  | sunset-30 → 21:00, or heavy weather mid-day |
| `rest`        | 56%        | 2237K  | 21:00 → 23:00, or heavy weather after sunset |
| `dim`         | 30%        | 2700K  | (reserved — currently unused in mood ternary) |
| `nightlight`  | 1%         | 2000K  | ≥ 23:00 or before sunrise+30min (lounge: also pre-sunrise+30) |
...
- `script.toggle_room` / `script.revert_room_to_auto` — parametric helpers (take `room_prefix`)
- `script.revert_all_to_auto` — clears every room's override and re-applies
- **`script.adaptive_<room>` (×6)** — parameterless wrappers, one per room. Mirrors wall-switch UX
  (off→clear override + apply sun state; on→freeze + off). Map these directly to Hue dimmer buttons,
  dashboard `tap_action`, voice assistants, NFC tags, etc. without filling in any field. Generated by
  `build_room_action_scripts.py`.
...
adaptive_lighting/
├── README.md                                  ← this file
├── build_button_automations.py                ← regenerates the 6 wall-switch automations
├── build_room_action_scripts.py               ← regenerates the 6 script.adaptive_<room> wrappers
└── payloads/
